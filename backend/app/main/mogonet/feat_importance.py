...
from app.main.mogonet.train_test import gen_trte_adj_mat, prepare_trte_data, test_epoch
...
def cal_feat_imp(data_folder, model_folder, view_list, num_class):
    ...
    f1 = f1_score(
        ...
        te_prob.argmax(1),
        average="macro" if num_class > 2 else None,
    )
    ...
    for i in range(len(featname_list)):
        ...
        for j in range(dim_list[i]):
            ...
            if np.isscalar(feat_imp_score):
                ...
                feat_imp["imp"][j] = feat_imp_score[
                    0
                ]  # Take the first element if it's a list or array
...
def summarize_imp_feat(featimp_list_list, data_folder):
    ...
    df_featimp_top = (
        df_featimp.groupby(["feat_name", "omics"])["imp"].sum().reset_index()
    )
