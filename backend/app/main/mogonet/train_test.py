...
from app.main.mogonet.utils import one_hot_tensor, cal_sample_weight, gen_adj_mat_tensor, gen_test_adj_mat_tensor, cal_adj_mat_parameter
...
def prepare_trte_data(data_folder, view_list):
    ...
    data_all_list = [torch.cat((data[idx_dict["tr"]].clone(), data[idx_dict["te"]].clone()), 0) for data in data_tensor_list]
...
def gen_trte_adj_mat(data_tr_list, data_trte_list, trte_idx, adj_parameter):
    ...
    for i in range(len(data_tr_list)):
        adj_param_adaptive = cal_adj_mat_parameter(adj_parameter, data_tr_list[i], adj_metric)
        adj_train_list.append(gen_adj_mat_tensor(data_tr_list[i], adj_param_adaptive, adj_metric))
        adj_test_list.append(gen_test_adj_mat_tensor(data_trte_list[i], trte_idx, adj_param_adaptive, adj_metric))
...
def train_epoch(data_list, adj_list, label, one_hot_label, sample_weight, model_dict, optim_dict, train_VCDN=True):
    ...
    criterion = torch.nn.CrossEntropyLoss(reduction='none')  # 损失函数：不自动求平均值，以便对样本权重进行加权
    ...
    if train_VCDN and num_view >= 2:
        ...
        ci_list = [model_dict["C{:}".format(i+1)](model_dict["E{:}".format(i+1)](data_list[i], adj_list[i])) for i in range(num_view)]
...
def test_epoch(data_list, adj_list, te_idx, model_dict):
    ...
    # 各视图的模型输出列表
    ci_list = [model_dict["C{:}".format(i+1)](model_dict["E{:}".format(i+1)](data_list[i], adj_list[i])) for i in range(len(data_list))]
...
def train_test(data_folder, view_list, num_class, lr_e_pretrain, lr_e, lr_c, num_epoch_pretrain, num_epoch):
    ...
    for epoch in range(num_epoch_pretrain):
        train_epoch(data_tr_list, adj_tr_list, labels_tr_tensor, onehot_labels_tr_tensor, sample_weight_tr, model_dict, optim_dict, train_VCDN=False)
    ...
    performance = {
        "training": [("Epoch", "ACC", "F1_Weighted", "F1_Macro", "AUC", "Precision", "Loss")],
        "testing": [("Epoch", "ACC", "F1_Weighted", "F1_Macro", "AUC", "Precision", "Loss")],
    }

    for epoch in range(num_epoch + 1):
        # Perform training for one epoch
        loss_dict = train_epoch(data_tr_list, adj_tr_list, labels_tr_tensor, onehot_labels_tr_tensor, sample_weight_tr, model_dict, optim_dict)
        epoch_loss = sum(loss_dict.values())  # Sum losses from all views

        for method in ["training", "testing"]:
            # Calculate performance metrics
            prob = test_epoch(data_tr_list if method == "training" else data_trte_list, adj_tr_list if method == "training" else adj_te_list, trte_idx[method[:2]], model_dict)  # Get probabilities for training data
            acc = accuracy_score(labels_trte[trte_idx[method[:2]]], prob.argmax(1))
            f1_weighted = f1_score(labels_trte[trte_idx[method[:2]]], prob.argmax(1), average='weighted')
            f1_macro = f1_score(labels_trte[trte_idx[method[:2]]], prob.argmax(1), average='macro')
            auc = roc_auc_score(labels_trte[trte_idx[method[:2]]], prob, multi_class="ovr") if num_class > 2 else roc_auc_score(labels_trte[trte_idx[method[:2]]], prob[:, 1])
            precision = precision_score(labels_trte[trte_idx[method[:2]]], prob.argmax(1), average='macro', zero_division=0)

            performance[method].append((epoch, acc, f1_weighted, f1_macro, auc, precision, epoch_loss))
