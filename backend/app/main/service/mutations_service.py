...
def get_mutation(name: str):
    ...
    params = {
        'terms': name,
        'df': 'MutationID,GeneName,PrimarySite'
    }
    response = requests.get(url, params=params)

    if response.status_code != 200:
        logging.getLogger("errors").warning(f"GET {response.url} failed with status {response.status_code}")
    ...
    output = {
        "COSMIC_data": [
            ...
            } for mutation_id, gene_name, primary_site in sorted(data[-1], key=lambda x: x[0])],
        "patients": []
    }
