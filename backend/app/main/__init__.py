...
def create_app(config_name: str) -> Flask:
    ...
    CORS(
        app,
        resources={
            r"/*": {"origins": ["http://localhost:8080", "http://192.168.209.129:8080"]}
        },
    ...
    def handle_options():
        if request.method == "OPTIONS":
            ...
            response.headers["Access-Control-Allow-Origin"] = (
                origin
                if origin in ["http://localhost:8080", "http://192.168.209.129:8080"]
                else None
            )
    ...
    def log_request(response):
        ...
        with db_get_cursor() as cur:
            ...
            details["user_id"] = (
                None if cur.fetchone() is not None else details["user_id"]
            )
            if payload:
                cur.execute("SELECT * FROM users WHERE id = %s;", (payload,))
                details["user_id"] = (
                    None if cur.fetchone() is None else details["user_id"]
                )
            cur.execute(
                """
