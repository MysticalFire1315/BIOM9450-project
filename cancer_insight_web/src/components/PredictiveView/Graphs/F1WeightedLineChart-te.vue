<!-- This is the trend chart for F1 weighted -->
<script setup>
...
const handleChartData = async () => {
    ...
    chartData.value = message.value.map(item => [item.epoch, item.f1_weighted]);
    updateChart();
};
...
const updateChart = () => {
    ...
    const option = {
        title: {
            text: 'testing Performance of F1 weighted Over Epochs',
            left: 20,
            top: 20,
        },
        grid: {
